...
@interface HPSphereView() <UIGestureRecognizerDelegate>

@end

@implementation HPSphereView
{
    NSMutableArray *tags;
    NSMutableArray *coordinate;
    HPPoint normalDirection;
    CGPoint last;
...
- (void)setCloudTags:(NSArray *)array
{
    ...
    for (NSInteger i = 0; i < tags.count; i ++) {
        ...
        HPPoint point = HPPointMake(x, y, z);
        NSValue *value = [NSValue value:&point withObjCType:@encode(HPPoint)];
    ...
    normalDirection = HPPointMake(a, b, 0);
    [self timerStart];
}

#pragma mark - set frame of point

- (void)updateFrameOfPoint:(NSInteger)index direction:(HPPoint)direction andAngle:(CGFloat)angle
{
    ...
    HPPoint rPoint = HPPointMakeRotation(point, direction, angle);
    value = [NSValue value:&rPoint withObjCType:@encode(HPPoint)];
...
- (void)setTagOfPoint: (HPPoint)point andIndex:(NSInteger)index
{
...
- (void)handlePanGesture:(UIPanGestureRecognizer *)gesture
{
    if (gesture.state == UIGestureRecognizerStateBegan) {
        ...
        HPPoint direction = HPPointMake(last.y - current.y, current.x - last.x, 0);
